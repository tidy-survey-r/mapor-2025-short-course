# Descriptive Analysis {background-color='{{< brand color secondary >}}'}
...
## `survey_prop()`: conditional proportions example {.smaller}
...
anes_des %>%
  group_by(Voted2024, CampaignInterest) %>%
  summarize(p = survey_prop())
```

## `survey_prop()`: joint proportions example {.smaller}
...
anes_des %>%
  group_by(interact(Voted2024, CampaignInterest)) %>%
  summarize(p = survey_prop())
```
